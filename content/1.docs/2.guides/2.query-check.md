...
| Check if a specific time range is free | `$model->isBookableAtTime('2025-01-15', '09:00', '10:00', null, 60, 15)` |
...
| `$bufferMinutes` | int\|null | null | Buffer between slots in minutes |
...
// Check if a 30-min slot at 09:45 is available, with a 15-min buffer enforced
$bookable = $user->isBookableAtTime('2025-01-15', '09:45', '10:15', null, 30, 15);
...
::tip
Pass the same `$bufferMinutes` value you use with `getBookableSlots()` to ensure both calls evaluate availability under identical conditions. Mismatched buffer values can cause `isBookableAtTime()` to accept a slot that `getBookableSlots()` would never generate.
::
